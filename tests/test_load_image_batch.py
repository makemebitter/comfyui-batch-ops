...
class TestSequentialBehavior:
    def test_starts_at_zero(self, image_dir):
        node = LoadImageBatch()
        _, _, idx, _ = _result(node.load_image(image_dir, batch_id='test1'))
        assert idx == 0
    ...
    def test_advances_each_call(self, image_dir):
        ...
        indices = []
        for _ in range(5):
            _, _, idx, _ = _result(node.load_image(image_dir, batch_id='test2'))
            indices.append(idx)
    ...
    def test_wraps_around(self, image_dir):
        ...
        indices = []
        for _ in range(7):
            _, _, idx, _ = _result(node.load_image(image_dir, batch_id='test3'))
            indices.append(idx)
        assert indices == [0, 1, 2, 3, 4, 0, 1]

    def test_different_batch_ids_independent(self, image_dir):
        node = LoadImageBatch()
        node.load_image(image_dir, batch_id='A')
        node.load_image(image_dir, batch_id='A')

        _, _, idx, _ = _result(node.load_image(image_dir, batch_id='B'))
        assert idx == 0

    def test_resets_on_path_change(self, image_dir):
        node = LoadImageBatch()
        node.load_image(image_dir, batch_id='reset_test')
        node.load_image(image_dir, batch_id='reset_test')

        d2 = tempfile.mkdtemp()
        try:
            Image.new('RGB', (32, 32), (0, 0, 0)).save(os.path.join(d2, 'img.png'))
            _, _, idx, _ = _result(node.load_image(d2, batch_id='reset_test'))
            assert idx == 0
        finally:
            shutil.rmtree(d2)
    ...
    def test_resets_on_filter_change(self, image_dir):
        node = LoadImageBatch()
        node.load_image(image_dir, batch_id='filt_test', filename_filter='*')
        node.load_image(image_dir, batch_id='filt_test', filename_filter='*')

        _, _, idx, _ = _result(node.load_image(
            image_dir, batch_id='filt_test', filename_filter='*.png'))
        assert idx == 0

    def test_total_images_count(self, image_dir):
        node = LoadImageBatch()
        _, _, _, total = _result(node.load_image(image_dir))
        assert total == 5

    def test_filename_with_extension(self, image_dir):
        node = LoadImageBatch()
        _, filename, _, _ = _result(node.load_image(image_dir, include_extension=True))
        assert '.' in filename

    def test_filename_without_extension(self, image_dir):
        node = LoadImageBatch()
        _, filename, _, _ = _result(node.load_image(image_dir, include_extension=False))
...
class TestUIOutput:
    def test_returns_status_text(self, image_dir):
        node = LoadImageBatch()
        rv = node.load_image(image_dir, batch_id='ui_test')
        text = rv["ui"]["text"][0]
        assert "1 / 5" in text

    def test_status_text_includes_filename(self, image_dir):
        node = LoadImageBatch()
        rv = node.load_image(image_dir, batch_id='ui_test2')
        text = rv["ui"]["text"][0]
        # first image alphabetically
        assert "apple.png" in text

    def test_no_preview_without_folder_paths(self, image_dir):
        """When folder_paths is unavailable, ui should still have text but no images."""
        with patch('nodes.load_image_batch.folder_paths', None):
            node = LoadImageBatch()
            rv = node.load_image(image_dir, batch_id='ui_test3')
...
class TestWidgetFeedback:
    def test_always_sends_index_feedback(self, image_dir):
        ...
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            node.load_image(image_dir, batch_id='fb1', unique_id='42')
    ...
    def test_feedback_advances_with_state(self, image_dir):
        ...
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            node.load_image(image_dir, batch_id='fb2', unique_id='42')
...
class TestAutoQueue:
    def test_requeues_mid_batch(self, image_dir):
        ...
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            node.load_image(image_dir, batch_id='aq1', auto_queue=True, unique_id='42')
    ...
    def test_stops_at_last_image(self, image_dir):
        ...
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            for _ in range(4):
                node.load_image(image_dir, batch_id='aq2', auto_queue=True, unique_id='42')
            mock_server.reset_mock()
    ...
    def test_full_batch_run(self, image_dir):
        """5 images, should re-queue 4 times then stop."""
        mock_server = MagicMock()
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            requeue_count = 0
            for _ in range(5):
                mock_server.reset_mock()
                node.load_image(image_dir, batch_id='aq_full',
    ...
    def test_json_persists_during_auto_queue(self, image_dir):
        """Resume works if browser disconnects mid-batch."""
        mock_server = MagicMock()
        with patch('nodes.load_image_batch.PromptServer', mock_server):
            node = LoadImageBatch()
            for _ in range(3):
                node.load_image(image_dir, batch_id='aq_persist',
        ...
        with patch('nodes.load_image_batch.PromptServer', None):
            ...
            _, _, idx, _ = _result(node2.load_image(
                image_dir, batch_id='aq_persist', auto_queue=True))
            assert idx == 3
    ...
    def test_no_crash_without_prompt_server(self, image_dir):
        with patch('nodes.load_image_batch.PromptServer', None):
            node = LoadImageBatch()
            _, _, idx, _ = _result(node.load_image(
                image_dir, batch_id='aq_no_server', auto_queue=True))
            assert idx == 0
...
class TestRGBAHandling:
    def test_rgba_auto_converted_to_rgb(self, rgba_image_dir):
        node = LoadImageBatch()
        image, _, _, _ = _result(node.load_image(rgba_image_dir))
        assert image.shape[3] == 3

    def test_rgb_stays_rgb(self, image_dir):
        node = LoadImageBatch()
        image, _, _, _ = _result(node.load_image(image_dir))
        assert image.shape[3] == 3
...
class TestErrorHandling:
    ...
    def test_no_matching_filter_raises(self, image_dir):
        node = LoadImageBatch()
        with pytest.raises(ValueError, match="No images found"):
            node.load_image(image_dir, filename_filter='*.xyz')
...
class TestIsChanged:
    def test_always_nan(self, image_dir):
        result = LoadImageBatch.IS_CHANGED(
            ...
            batch_id='test', auto_queue=False,
            index=0, include_extension=True
        )
